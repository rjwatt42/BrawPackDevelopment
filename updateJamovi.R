...
dir1<-"/Users/rogerwatt/Documents/GitHub/BrawStats-for-Jamovi-Full/R"
dir2<-"/Users/rogerwatt/Documents/GitHub/JamoviDevelopment/R"
...
for (fi in 1:length(fint)) {
  ...
  if (!isempty(use1) && !isempty(use2)) {
    ...
    if (mismatch) {
      ...
      if (fdate1>fdate2) {
        print(paste0("Update ",fint[fi]," from BrawStats-for-Jamovi-Full (",format(fdate1-fdate2,digits=1),") ",output))
      }
      if (fdate2>fdate1) {
        print(paste0("Update ",fint[fi]," from JamoviDevelopment (",format(fdate2-fdate1,digits=1),") ",output))
      }
    }
  }
}
